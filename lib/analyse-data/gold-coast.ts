...
export type GoldCoastSuburbFactors = LocationFactors
...
/** UI labels — derived from `BUSINESS_TYPE_WEIGHTS` (single source of truth). */
export const GOLD_COAST_SCORE_WEIGHTS = {
  cafe: {
    demand: BUSINESS_TYPE_WEIGHTS.cafe.demandStrength!,
    rent: BUSINESS_TYPE_WEIGHTS.cafe.rentPressure!,
    competition: BUSINESS_TYPE_WEIGHTS.cafe.competitionDensity!,
    seasonality: BUSINESS_TYPE_WEIGHTS.cafe.seasonalityRisk!,
  },
  restaurant: {
    demand: BUSINESS_TYPE_WEIGHTS.restaurant.demandStrength!,
    rent: BUSINESS_TYPE_WEIGHTS.restaurant.rentPressure!,
    competition: BUSINESS_TYPE_WEIGHTS.restaurant.competitionDensity!,
    seasonality: BUSINESS_TYPE_WEIGHTS.restaurant.seasonalityRisk!,
    tourism: BUSINESS_TYPE_WEIGHTS.restaurant.tourismDependency!,
  },
  retail: {
    demand: BUSINESS_TYPE_WEIGHTS.retail.demandStrength!,
    rent: BUSINESS_TYPE_WEIGHTS.retail.rentPressure!,
    tourism: BUSINESS_TYPE_WEIGHTS.retail.tourismDependency!,
    competition: BUSINESS_TYPE_WEIGHTS.retail.competitionDensity!,
    seasonality: BUSINESS_TYPE_WEIGHTS.retail.seasonalityRisk!,
  },
} as const

export const GOLD_COAST_VERDICT_THRESHOLDS = VERDICT_SCORE_THRESHOLDS

export const GOLD_COAST_FACTOR_META = FACTOR_META
...
export function getGoldCoastCompositeScore(cafe: number, restaurant: number, retail: number): number {
  return computeCompositeScore({ cafe, restaurant, retail })
}
...
export function deriveGoldCoastVerdict(
  ...
  return deriveVerdictFromComposite(getGoldCoastCompositeScore(cafe, restaurant, retail))
}
...
const GOLD_COAST_SUBURBS_DERIVED: GoldCoastSuburb[] = GOLD_COAST_SUBURBS.map((suburb) => {
  ...
  const m = computeLocationModel(suburb.factors)
  return {
    ...suburb,
    cafe: m.cafe,
