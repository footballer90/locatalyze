...
import {
  getMelbourneSuburb,
  getMelbourneSuburbSlugs,
  getMelbourneNearbySuburbs,
  type SuburbModel,
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getMelbourneNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <section style={{ borderBottom: `1px solid ${C.border}`, background: C.white, padding: '56px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 13, color: C.muted, marginBottom: 18 }}>
            ...
            <span>›</span>
            <Link href="/analyse/melbourne" style={{ color: C.brand, textDecoration: 'none' }}>Melbourne</Link>
            <span>›</span>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Melbourne dataset. You can run a full analysis on any address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/melbourne" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Melbourne suburbs</h2>
...
function SuburbSchema({ suburb }: { suburb: SuburbModel }) {
  const schema = {
    ...
    about: {
      '@type': 'Place',
      name: `${suburb.name}, Melbourne VIC`,
      address: {
        '@type': 'PostalAddress',
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) {
    return {
      title: 'Melbourne suburb not found — Locatalyze',
      description: 'This Melbourne suburb is not in the current analysis dataset.',
    }
  }
...
export default async function MelbourneSuburbPage({ params }: Props) {
  ...
  const nearby = getMelbourneNearbySuburbs(data.slug, 3)
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      ...
      {/* ── Hero ── */}
      <section style={{ background: 'linear-gradient(135deg, #0369A1 0%, #024F80 100%)', color: C.white, padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          {/* Breadcrumb */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/melbourne" style={{ color: 'inherit', textDecoration: 'none' }}>Melbourne</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            ...
            <div>
              ...
              <p style={{ maxWidth: 560, fontSize: 15, lineHeight: 1.7, color: '#E0F2FE', margin: '0 0 20px 0' }}>
                {data.why[0]}
              </p>
      ...
      <section style={{ padding: '32px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
          ...
          {/* CTA */}
          <div style={{ background: 'linear-gradient(135deg, #047857 0%, #059669 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
            ...
            <Link href="/onboarding" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: C.white, color: C.emerald, borderRadius: 10, textDecoration: 'none', fontSize: 15, fontWeight: 800 }}>
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/melbourne" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>
              ← Back to Melbourne overview
            </Link>
          </div>
        </div>
