...
// VERSION 2 — engine-driven scores (getDarwinSuburbs().retail) replacing hand-entered values.
// Narrative, charts, poll, and checklist sections retained; no score drift vs hub page.
...
import { getDarwinSuburbs } from '@/lib/analyse-data/darwin'

// ── Engine data ────────────────────────────────────────────────────────────────
// Single source of truth: same dataset as /analyse/darwin hub page.
// Scores are computed by computeLocationModel() in scoring-engine.ts.
const _suburbs = getDarwinSuburbs()
function getRetailScore(name: string): number {
  return _suburbs.find(s => s.name === name)?.retail ?? 0
}
function getRetailVerdict(name: string): 'GO' | 'CAUTION' | 'NO' {
  const score = getRetailScore(name)
  if (score >= 69) return 'GO'
...
const SCHEMAS = [
  ...
  },
  {
    ...
    mainEntity: [
      ...
      { '@type': 'Question', name: 'Which Darwin suburbs should I avoid for retail?', acceptedAnswer: { '@type': 'Answer', text: 'Berrimah (industrial/warehouse zone), Humpty Doo (rural fringe, dispersed population), and Winnellie (light industrial, no residential density) should be avoided. These lack retail foot traffic and are viable only for wholesale operations. Casuarina scores low for independent retail specifically because Casuarina Square mall dominates the catchment — independents face structural disadvantage competing against entrenched mall gravity.' } },
    ],
  },
]

// ── Chart data — scores derived from engine ────────────────────────────────────
const SUBURB_SCORES = [
  { suburb: 'Parap',       score: getRetailScore('Parap'),       rent: 3150, traffic: 74, income: 92 },
  { suburb: 'Mitchell St', score: getRetailScore('Darwin City'), rent: 5000, traffic: 92, income: 82 },
  { suburb: 'Rapid Creek', score: getRetailScore('Rapid Creek'), rent: 2400, traffic: 58, income: 75 },
  { suburb: 'Stuart Park', score: getRetailScore('Stuart Park'), rent: 2850, traffic: 68, income: 78 },
  { suburb: 'Palmerston',  score: getRetailScore('Palmerston'),  rent: 1900, traffic: 48, income: 72 },
  { suburb: 'Casuarina',   score: getRetailScore('Casuarina'),   rent: 4100, traffic: 88, income: 88 },
]
...
// ── Poll data ──────────────────────────────────────────────────────────────────
const POLL_OPTIONS = [
  { label: 'Parap', initial: 38 },
  { label: 'Mitchell Street', initial: 29 },
  { label: 'Rapid Creek', initial: 14 },
  { label: 'Stuart Park', initial: 12 },
  { label: 'Somewhere else', initial: 7 },
]

// ── Suburb data — scores from engine, narrative retained ──────────────────────
const TOP_SUBURBS = [
  {
    rank: 1,
    name: 'Parap',
    postcode: '0820',
    score: getRetailScore('Parap'),
    verdict: getRetailVerdict('Parap'),
  ...
  {
    rank: 2,
    name: 'Mitchell Street CBD',
    postcode: '0800',
    score: getRetailScore('Darwin City'),
    verdict: getRetailVerdict('Darwin City'),
  ...
  },
  {
    ...
    name: 'Stuart Park',
    postcode: '0820',
    score: getRetailScore('Stuart Park'),
    verdict: getRetailVerdict('Stuart Park'),
    ...
    angle: 'Emerging location between CBD and suburbs, foot traffic improving as residential base grows',
    detail: [
  ...
  },
  {
    ...
    name: 'Casuarina',
    postcode: '0810',
    score: getRetailScore('Casuarina'),
    verdict: getRetailVerdict('Casuarina'),
    income: '$88,000', rent: '$3,200–$5,000/mo', competition: '4 within 500m + mall',
    footTraffic: 88, demographics: 85, rentFit: 60, competitionScore: 38,
    breakEven: '48/day', payback: '12 months', annualProfit: '$94,000',
    angle: 'Highest raw foot traffic in northern suburbs — but independents compete against entrenched Casuarina Square mall gravity',
    detail: [
    ...
    risks: 'Mall competition is the defining constraint. Independent retail in Casuarina faces Casuarina Square as a direct competitor for the same customer base — that\'s a 60,000 sqm mall with anchor tenants and car parking. Air conditioning is non-negotiable operating cost at $800–$1,200/month. Wet season revenue still drops 35–40% despite indoor traffic.',
    opportunity: 'Specialty retail with clear positioning that the mall cannot replicate (local makers, highly differentiated concept, services-adjacent retail) can exploit the foot traffic while sidestepping direct mall competition. The opportunity is niche differentiation, not general retail.',
  },
]
      ...
      <div style={{ background: 'linear-gradient(135deg, #78350F 0%, #92400E 50%, #B45309 100%)', padding: '60px 24px 52px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(217,119,6,0.15)', border: '1px solid rgba(217,119,6,0.3)', borderRadius: 100, padding: '5px 14px', fontSize: 11, fontWeight: 700, color: '#D97706', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: 18 }}>
            Darwin Retail Location Guide · Updated April 2026
          </div>
          ...
          <div style={{ display: 'flex', gap: 28, marginTop: 36, paddingTop: 28, borderTop: '1px solid rgba(255,255,255,0.1)', flexWrap: 'wrap' as const }}>
            {[{ v: '6', l: 'Darwin suburbs scored' }, { v: '150k', l: 'City population' }, { v: 'Apr 2026', l: 'Last updated' }].map(({ v, l }) => (
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '52px 24px 80px' }}>
        ...
        <section style={{ marginBottom: 44 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14 }}>
            {[
              ...
              { stat: '0', claim: 'Westfield-anchored shopping centres — Darwin has zero, creating whitespace for well-positioned independents', source: 'Australian Retail Council Centre Database 2025', color: '#D97706' },
        ...
        <section style={{ marginBottom: 44 }}>
          ...
          <div style={{ background: S.white, border: `1px solid ${S.border}`, borderRadius: 14, padding: '24px 20px', marginTop: 20, marginBottom: 8 }}>
            ...
            <p style={{ fontSize: 12, color: S.muted, marginBottom: 16 }}>Bubble size = rent/revenue ratio. Points in the green zone have rent below 10% of revenue (Darwin benchmark).</p>
        ...
        {/* Bar chart: suburb scores from engine */}
        <section id="suburbs" style={{ marginBottom: 44 }}>
          ...
          <p style={{ fontSize: 14, color: S.muted, marginBottom: 18 }}>Scores ≥ 69 = GO. 60–68 = CAUTION. Below 60 = RISKY.</p>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={SUBURB_SCORES} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip content={({ payload, label }) => {
                    if (!payload?.length) return null
                    const d = payload[0].payload
                    const verdict = d.score >= 69 ? 'GO' : d.score >= 60 ? 'CAUTION' : 'RISKY'
                    const color = d.score >= 69 ? S.emerald : d.score >= 60 ? S.amber : S.red
                    return (
                      <div style={{ background: S.white, border: `1px solid ${S.border}`, borderRadius: 10, padding: '10px 14px', fontSize: 12 }}>
                        <p style={{ fontWeight: 700, color: S.n900, marginBottom: 4 }}>{label}</p>
                        <p style={{ color, fontWeight: 700 }}>{verdict} — {d.score}/100 retail</p>
                        <p style={{ color: S.muted }}>Income: ${d.income}k median</p>
                        <p style={{ color: S.muted }}>Foot traffic index: {d.traffic}/100</p>
                      </div>
                    )
                  }} />
                  <Bar dataKey="score" radius={[6, 6, 0, 0]} maxBarSize={56}
                    fill={S.brand}
                    label={{ position: 'top', fontSize: 12, fontWeight: 700, fill: '#64748B', formatter: (v: unknown) => `${v}` }}
                    isAnimationActive={true}>
                    {SUBURB_SCORES.map((entry, index) => (
                      <rect key={index} fill={entry.score >= 69 ? S.emerald : entry.score >= 60 ? S.amber : S.red} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <DataNote text="Scores: Locatalyze 5-factor engine (demand 28%, rent 22%, tourism 22%, competition 18%, seasonality 10%). Same model as /analyse/darwin hub — no separate manual scoring. ABS 2024–26, Darwin property listings, Geoapify Places API. April 2026." />
        </section>
        ...
        <section style={{ marginBottom: 44 }}>
          ...
          {TOP_SUBURBS.map((sub, idx) => {
            const verdictColor = sub.verdict === 'GO' ? S.emerald : sub.verdict === 'CAUTION' ? S.amber : S.red
            return (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 20, alignItems: 'start' }}>
                  ...
                  <div style={{ minWidth: 160 }}>
                    <div style={{ textAlign: 'center', marginBottom: 14 }}>
                      <div style={{ fontSize: 52, fontWeight: 900, color: verdictColor, lineHeight: 1 }}>{sub.score}</div>
                      <div style={{ fontSize: 11, color: S.muted }}>/100 retail</div>
                    </div>
                    <ScoreBar label="Foot Traffic" value={sub.footTraffic} color={verdictColor} />
                    <ScoreBar label="Area Demographics" value={sub.demographics} color={verdictColor} />
                    <ScoreBar label="Rent Affordability" value={sub.rentFit} color={verdictColor} />
                    <ScoreBar label="Competition" value={sub.competitionScore} color={verdictColor} />
                  </div>
                </div>
              </div>
            )
        ...
        <section style={{ marginBottom: 44 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
            {[
              { title: 'Foot traffic volume', weight: '28% of retail score', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={S.brand} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" /></svg>, detail: 'Darwin retail lives on destination foot traffic — anchor shopping centres are absent in most suburbs. Parap Saturday market, Mitchell Street precinct, and suburban strips drive baseline volumes. Visit your location on Wednesday at 10am and Saturday at 11am; count pedestrians for 30 minutes each. The weekend surge reveals true repeat-customer capacity.' },
              { title: 'Median household income', weight: '22% of retail score (via demand)', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={S.brand} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23" /><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" /></svg>, detail: 'Darwin\'s $98,000 median household income creates a spending profile unlike smaller Australian cities. Defence sector, government employment, and mining/LNG wages concentrate discretionary income. At this income level, quality retail concept with clear positioning captures category loyalty. A generic discount retailer struggles against online alternatives; differentiated concept thrives.' },
              { title: 'Seasonal revenue modelling', weight: '10% of retail score (direct)', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={S.brand} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="6" /><circle cx="12" cy="12" r="2" /></svg>, detail: 'Wet season (November–April) creates 40% revenue drop that southern retailers never model. A $58,000/month dry season revenue becomes $35,000/month November–April. Budget cashflow for two distinct seasons. Build 6-month cash reserves or risk insolvency despite strong overall viability.' },
              { title: 'Air conditioning infrastructure', weight: 'Fixed operating cost ~$1k/mo', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={S.brand} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg>, detail: 'Air conditioning is not a luxury in Darwin — it is retail infrastructure. Full-service AC is mandatory for customer comfort during dry season (32+°C heat) and absolutely required during wet season (extreme humidity). Budget $800–$1,200/month AC operating costs. This is a fixed cost that does not scale with revenue. Location with existing efficient AC systems is a material economic advantage.' },
            ].map(f => (
              ...
              <div key={i} style={{ background: S.white, border: `1px solid ${S.border}`, borderRadius: 18, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '24px 28px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(100px, 1fr))', gap: 10, marginBottom: 8 }}>
                    {[['Monthly rent', `$${cs.rent.toLocaleString()}`, S.muted], ['Monthly revenue', `$${cs.revenue.toLocaleString()}`, S.emerald], ['Customers/day', `${cs.customers}`, S.brand], ['Margin', `${cs.margin}%`, S.emerald], ['Annual profit', `$${cs.annual.toLocaleString()}`, S.emerald], ['Payback', `${cs.payback} months`, S.amber]].map(([l, v, c]) => (
        ...
        <section style={{ marginBottom: 44 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, background: S.white, minWidth: 640 }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${S.border}`, background: S.n50 }}>
                  {['Suburb', 'Retail Score', 'Verdict', 'Median Income', 'Rent Range', 'Competition', 'Est. Payback'].map(h => (
          ...
          <DataNote text="Retail scores: Locatalyze 5-factor engine — same model as /analyse/darwin hub. Income: ABS 2023–24. Rent: Darwin property listings Q1 2026. Payback: Locatalyze model, $140k setup, IBISWorld COGS benchmarks, dry season baseline." />
        </section>

        {/* FAQ */}
        <section style={{ marginBottom: 44 }}>
          <h2 style={{ fontSize: 26, fontWeight: 900, color: S.n900, letterSpacing: '-0.03em', marginBottom: 16 }}>Frequently Asked Questions</h2>
          {(SCHEMAS[1] as any).mainEntity.map(({ name, acceptedAnswer }: { name: string; acceptedAnswer: { text: string } }) => (
