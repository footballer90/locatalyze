...
function bestFitLabel(suburb: SuburbModel) {
  const scores = [
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getPerthNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <section style={{ borderBottom: `1px solid ${C.border}`, background: C.white, padding: '56px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 13, color: C.muted, marginBottom: 18 }}>
            ...
            <span>›</span>
            <Link href="/analyse/perth" style={{ color: C.brand, textDecoration: 'none' }}>Perth</Link>
            <span>›</span>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Perth dataset. You can run a full analysis on any Perth address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/perth" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Perth suburbs</h2>
...
function SuburbSchema({ suburb }: { suburb: SuburbModel }) {
  const schema = {
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) {
    return {
      title: 'Perth suburb not found — Locatalyze',
      description: 'This Perth suburb is not in the current analysis dataset.',
    }
  }
...
export default async function PerthSuburbPage({ params }: Props) {
  ...
  const nearby = getPerthNearbySuburbs(data.slug, 3)
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      ...
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #022C22 0%, #064E3B 50%, #0F766E 100%)', color: C.white, padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/perth" style={{ color: 'inherit', textDecoration: 'none' }}>Perth</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#A7F3D0', margin: '0 0 12px 0' }}>
                Perth Suburb Intelligence
              </p>
              <h1 style={{ fontSize: 'clamp(34px, 6vw, 54px)', lineHeight: 1.03, margin: '0 0 14px 0' }}>
                {data.name}
              </h1>
              <p style={{ maxWidth: 560, fontSize: 15, lineHeight: 1.7, color: '#D1FAE5', margin: '0 0 20px 0' }}>
                {data.why[0]}
              </p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 14, color: '#A7F3D0' }}>
                  Best fit: {bestFit.label} ({bestFit.value}/100)
                </span>
              </div>
            </div>

            <div style={{ background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.16)', borderRadius: 18, padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 18, marginBottom: 20 }}>
                <div>
                  <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#A7F3D0', margin: '0 0 6px 0' }}>Composite score</p>
                  <div style={{ fontSize: 64, lineHeight: 0.92, fontWeight: 900 }}>{data.compositeScore}</div>
                  <div style={{ fontSize: 13, color: '#A7F3D0', marginTop: 6 }}>out of 100</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#A7F3D0', margin: '0 0 6px 0' }}>Verdict</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                {[
                  ...
                  <div key={label} style={{ textAlign: 'center', padding: '10px 6px', background: 'rgba(255,255,255,0.10)', borderRadius: 10 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#A7F3D0', marginTop: 4 }}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            ...
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 26 }}>
              ...
              <p style={{ fontSize: 12, color: C.mutedLight, lineHeight: 1.6, margin: '14px 0 0 0' }}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              {data.why.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ flexShrink: 0, width: 28, height: 28, borderRadius: '50%', background: C.amberBg, border: `1px solid ${C.amberBdr}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
                    <span style={{ fontSize: 12, fontWeight: 900, color: C.amber }}>{i + 1}</span>
                  </div>
                  <p style={{ fontSize: 15, lineHeight: 1.7, color: C.n800, margin: 0 }}>{item}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* CTA */}
          <div style={{ background: 'linear-gradient(135deg, #022C22 0%, #064E3B 60%, #0F766E 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: '#F0FDF9', margin: '0 0 10px 0' }}>
              Have a specific address in {data.name}?
            </h2>
            <p style={{ fontSize: 15, color: 'rgba(167,243,208,0.8)', margin: '0 0 20px 0', lineHeight: 1.65 }}>
              Run a full competitor map, rent benchmark, and GO/CAUTION/NO verdict for any {data.name} address. Free.
            </p>
            <Link href="/onboarding" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: '#34D399', color: '#022C22', borderRadius: 10, textDecoration: 'none', fontSize: 15, fontWeight: 800 }}>
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/perth" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>
              ← Back to Perth overview
            </Link>
          </div>
        </div>
