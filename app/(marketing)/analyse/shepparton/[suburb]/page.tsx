...
import {
  getSheppartonSuburb,
  getSheppartonSuburbSlugs,
  getSheppartonNearbySuburbs,
  type SheppartonSuburb,
} from '@/lib/analyse-data/shepparton'
...
function bestFitLabel(suburb: SheppartonSuburb) {
  return [
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getSheppartonNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <section style={{ borderBottom: `1px solid ${C.border}`, background: C.white, padding: '56px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 13, color: C.muted, marginBottom: 18 }}>
            ...
            <span>›</span>
            <Link href="/analyse/shepparton" style={{ color: C.brand, textDecoration: 'none' }}>Shepparton</Link>
            <span>›</span>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: C.brand, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px 0' }}>Shepparton Suburb Intelligence</p>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Shepparton dataset. You can run a full analysis on any address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/shepparton" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>Back to Shepparton overview</Link>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Shepparton suburbs</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {alternatives.map((s) => (
              <SuburbCard key={s.slug} citySlug="shepparton" name={s.name} slug={s.slug} description={s.why[0]} score={s.compositeScore} verdict={s.verdict === 'RISKY' ? 'NO' : s.verdict} rentRange="" />
            ))}
          </div>
        </div>
      </section>
...
function SuburbSchema({ suburb }: { suburb: SheppartonSuburb }) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: `${suburb.name} Shepparton Business Location Analysis — ${suburb.verdict} (${suburb.compositeScore}/100)`,
    description: `${suburb.name} scores ${suburb.compositeScore}/100 for Shepparton business suitability with a ${suburb.verdict} verdict for cafes, restaurants, and retail.`,
    author: { '@type': 'Organization', name: 'Locatalyze' },
    publisher: { '@type': 'Organization', name: 'Locatalyze', url: 'https://www.locatalyze.com' },
    dateModified: '2026-04-21',
    about: {
      '@type': 'Place',
      name: `${suburb.name}, Shepparton VIC`,
...
export function generateStaticParams() {
  return getSheppartonSuburbSlugs().map((slug) => ({ suburb: slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { suburb } = await params
  const data = getSheppartonSuburb(decodeURIComponent(suburb))
  if (!data) return { title: 'Shepparton suburb not found — Locatalyze', description: 'This Shepparton suburb is not in the current analysis dataset.' }
  return {
    title: `${data.name} Shepparton Business Analysis — ${data.verdict} (${data.compositeScore}/100)`,
    ...
    alternates: { canonical: `https://www.locatalyze.com/analyse/shepparton/${data.slug}` },
    openGraph: {
      title: `${data.name} Shepparton Business Analysis`,
      description: `${data.name} scores ${data.compositeScore}/100 — cafe ${data.cafe}, restaurant ${data.restaurant}, retail ${data.retail}.`,
      url: `https://www.locatalyze.com/analyse/shepparton/${data.slug}`,
      type: 'article',
    },
  }
}

export default async function SheppartonSuburbPage({ params }: Props) {
  const { suburb } = await params
  const data = getSheppartonSuburb(decodeURIComponent(suburb))
  ...
  const nearby = getSheppartonNearbySuburbs(data.slug, 3)

  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <SuburbSchema suburb={data} />

      {/* ── Hero — orchard green gradient for Shepparton ── */}
      <section style={{ background: 'linear-gradient(135deg, #14532D 0%, #15803D 50%, #16A34A 100%)', color: '#FFFFFF', padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/shepparton" style={{ color: 'inherit', textDecoration: 'none' }}>Shepparton</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#DCFCE7', margin: '0 0 12px 0' }}>Shepparton Suburb Intelligence</p>
      ...
      <section style={{ padding: '32px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #14532D 0%, #15803D 50%, #16A34A 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
            ...
            <Link href="/onboarding" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: '#DCFCE7', color: '#14532D', borderRadius: 10, textDecoration: 'none', fontSize: 15, fontWeight: 800 }}>
              Analyse your {data.name} address →
            </Link>
          </div>

          <div>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: C.n900, margin: '0 0 16px 0' }}>Other Shepparton suburbs to consider</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
              {nearby.map((s) => (
                <SuburbCard key={s.slug} citySlug="shepparton" name={s.name} slug={s.slug} description={s.why[0]} score={s.compositeScore} verdict={s.verdict === 'RISKY' ? 'NO' : s.verdict} rentRange="" />
              ))}
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/shepparton" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>← Back to Shepparton overview</Link>
          </div>
        </div>
      </section>
    </div>
