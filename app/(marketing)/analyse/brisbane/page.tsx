...
function bScore(name: string): number {
  return getBrisbaneSuburb(name)?.compositeScore ?? 0
}
function bVerdict(name: string): 'GO' | 'CAUTION' | 'NO' {
  const v = getBrisbaneSuburb(name)?.verdict
  if (v === 'GO') return 'GO'
...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    url: 'https://locatalyze.com/analyse/brisbane',
  },
}

const COMPARISON_ROWS = [
  { name: 'Paddington', score: bScore('Paddington'), verdict: bVerdict('Paddington'), rent: '$4,500–$8,000', footTraffic: 'Very High', bestFor: 'Premium hospitality, boutique retail, café' },
  { name: 'West End', score: bScore('West End'), verdict: bVerdict('West End'), rent: '$5,000–$9,000', footTraffic: 'Very High', bestFor: 'Independent cafés, hospitality, creative' },
  { name: 'New Farm', score: bScore('New Farm'), verdict: bVerdict('New Farm'), rent: '$4,000–$7,500', footTraffic: 'High', bestFor: 'Brunch cafés, riverfront dining, premium' },
  { name: 'Woolloongabba', score: bScore('Woolloongabba'), verdict: bVerdict('Woolloongabba'), rent: '$3,500–$6,500', footTraffic: 'High', bestFor: 'Hospitality, stadium ancillary, health' },
  { name: 'Bulimba', score: bScore('Bulimba'), verdict: bVerdict('Bulimba'), rent: '$3,000–$5,500', footTraffic: 'Medium-High', bestFor: 'Premium casual, café, professional services' },
  { name: 'Kangaroo Point', score: bScore('Kangaroo Point'), verdict: bVerdict('Kangaroo Point'), rent: '$3,000–$5,500', footTraffic: 'Medium-High', bestFor: 'Riverfront dining, wellness, café' },
  { name: 'Brisbane CBD', score: bScore('Brisbane CBD'), verdict: bVerdict('Brisbane CBD'), rent: '$12,000–$28,000', footTraffic: 'Very High', bestFor: 'Premium/luxury, high-volume only' },
  { name: 'Chermside', score: bScore('Chermside'), verdict: bVerdict('Chermside'), rent: '$3,000–$5,500', footTraffic: 'High', bestFor: 'Strip retail adjacent to centre only' },
]
...
const SUBURB_CATEGORIES = [
  {
    ...
    suburbs: [
      { name: 'Paddington', slug: 'paddington', description: "Brisbane\'s benchmark for independent hospitality. Given Terrace produces more successful independent businesses per metre than anywhere else in Queensland.", score: bScore('Paddington'), verdict: bVerdict('Paddington'), rentRange: '$4,500–$8,000/mo' },
      { name: 'West End', slug: 'west-end', description: 'Boundary Street indie culture. Educated 25–45 demographic actively supports independent operators over chains.', score: bScore('West End'), verdict: bVerdict('West End'), rentRange: '$5,000–$9,000/mo' },
      { name: 'New Farm', slug: 'new-farm', description: "Brisbane\'s brunch capital. Only 3 independent cafés despite exceptional demand — rare first-mover gap in a premium market.", score: bScore('New Farm'), verdict: bVerdict('New Farm'), rentRange: '$4,000–$7,500/mo' },
      { name: 'South Brisbane', slug: 'south-brisbane', description: 'Cultural precinct adjacent to GOMA and QPAC. Tourist-professional mix; Weekend trade very strong, weekday more variable.', score: bScore('South Brisbane'), verdict: bVerdict('South Brisbane'), rentRange: '$5,500–$10,000/mo' },
    ],
  },
  {
    ...
    suburbs: [
      { name: 'Woolloongabba', slug: 'woolloongabba', description: 'The Gabba precinct is in structural transition. Post-Olympic infrastructure driving rapid gentrification. Best momentum in Brisbane.', score: bScore('Woolloongabba'), verdict: bVerdict('Woolloongabba'), rentRange: '$3,500–$6,500/mo' },
      { name: 'Bulimba', slug: 'bulimba', description: "Oxford Street\'s overlooked premium. Affluent residential base comparable to Paddington but without the operator attention.", score: bScore('Bulimba'), verdict: bVerdict('Bulimba'), rentRange: '$3,000–$5,500/mo' },
      { name: 'Kangaroo Point', slug: 'kangaroo-point', description: 'Riverside professional enclave. Story Bridge and ferry access drive weekend trade; weekday professional base growing.', score: bScore('Kangaroo Point'), verdict: bVerdict('Kangaroo Point'), rentRange: '$3,000–$5,500/mo' },
      { name: 'Teneriffe', slug: 'teneriffe', description: 'Wool store conversions created a distinct character precinct. Riverside positioning and professional demographic with strong spending power.', score: bScore('Teneriffe'), verdict: bVerdict('Teneriffe'), rentRange: '$3,500–$6,500/mo' },
      { name: 'Fortitude Valley', slug: 'fortitude-valley', description: 'Beyond the nightlife reputation. Brunswick and Ann Street daytime trade has materially improved over five years. High rents compress margins — entry requires clear differentiation.', score: bScore('Fortitude Valley'), verdict: bVerdict('Fortitude Valley'), rentRange: '$4,000–$7,000/mo' },
      { name: 'Toowong', slug: 'toowong', description: 'University of Queensland gateway suburb. Student-professional mix with underserved quality food and services market. Semester-driven seasonality applies.', score: bScore('Toowong'), verdict: bVerdict('Toowong'), rentRange: '$3,000–$5,500/mo' },
    ],
  },
  {
    ...
    suburbs: [
      { name: 'Greenslopes', slug: 'greenslopes', description: 'Hospital district with consistent demand. Allied health and professional services structurally underserved despite strong catchment.', score: bScore('Greenslopes'), verdict: bVerdict('Greenslopes'), rentRange: '$2,500–$4,500/mo' },
      { name: 'Newmarket', slug: 'newmarket', description: 'Enoggera Road corridor improving steadily. Younger professional demographic and lower rents than inner suburbs.', score: bScore('Newmarket'), verdict: bVerdict('Newmarket'), rentRange: '$2,500–$4,500/mo' },
      { name: 'Nundah', slug: 'nundah', description: 'Inner north on the move. Sandgate Road and Nundah Village strip have strong community-driven café culture.', score: bScore('Nundah'), verdict: bVerdict('Nundah'), rentRange: '$2,500–$4,500/mo' },
      { name: 'Mount Gravatt', slug: 'mount-gravatt', description: 'Griffith University anchor. Student and family demographic creates consistent demand at accessible rent levels.', score: bScore('Mount Gravatt'), verdict: bVerdict('Mount Gravatt'), rentRange: '$2,500–$4,500/mo' },
      { name: 'Indooroopilly', slug: 'indooroopilly', description: 'Westfield and UQ proximity. Strip retail outside the centre performs when positioned correctly for the academic-family mix.', score: bScore('Indooroopilly'), verdict: bVerdict('Indooroopilly'), rentRange: '$2,500–$4,500/mo' },
      { name: 'Carindale', slug: 'carindale', description: 'Southeast Brisbane family anchor. Westfield Carindale drives foot traffic; family health and casual dining are reliable categories.', score: bScore('Carindale'), verdict: bVerdict('Carindale'), rentRange: '$2,800–$5,200/mo' },
      { name: 'Annerley', slug: 'annerley', description: 'Ipswich Road corridor improving. Healthcare and multicultural food are the established categories; café culture emerging.', score: bScore('Annerley'), verdict: bVerdict('Annerley'), rentRange: '$2,000–$3,800/mo' },
    ],
  },
  {
    ...
    suburbs: [
      { name: 'Brisbane CBD', slug: 'brisbane-cbd', description: 'Queen Street Mall foot traffic is strong but $15,000+ rents demand extraordinary volume. Post-pandemic office recovery at 70–75% capacity.', score: bScore('Brisbane CBD'), verdict: bVerdict('Brisbane CBD'), rentRange: '$12,000–$28,000/mo' },
      { name: 'Spring Hill', slug: 'spring-hill', description: 'Office and residential mix. Weekend population thin. Works for weekday corporate lunch concepts; evenings and weekends underperform.', score: bScore('Spring Hill'), verdict: bVerdict('Spring Hill'), rentRange: '$5,000–$9,000/mo' },
      { name: 'Chermside', slug: 'chermside', description: 'Westfield Chermside monopoly effect. Independent operators outside the centre face chain saturation — strip retail economics are structurally challenged.', score: bScore('Chermside'), verdict: bVerdict('Chermside'), rentRange: '$3,000–$5,500/mo' },
    ],
  },
]
      ...
      <section id="top-20" style={{ padding: '56px 24px', backgroundColor: '#FFFFFF' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
            ...
            {[
              { name: 'Paddington', slug: 'paddington', description: 'Given Terrace is Queensland\'s most successful independent hospitality strip. Five venues that compete nationally. Vacancy is low — when a tenancy comes up, act fast. Rents of $4,500–$8,000/month are Brisbane\'s highest for strip retail, but justified by customer quality and spending patterns.', score: bScore('Paddington'), verdict: bVerdict('Paddington'), rentRange: '$4,500–$8,000/mo' },
              { name: 'West End', slug: 'west-end', description: 'Boundary Street is Brisbane\'s version of Fitzroy — an independent hospitality strip with educated demographics and strong community loyalty to local operators. Less concentrated than Paddington, which means more opportunity for new entrants. Rents $5,000–$9,000/month, with secondary Vulture Street positions at $3,000–$5,000.', score: bScore('West End'), verdict: bVerdict('West End'), rentRange: '$5,000–$9,000/mo' },
              { name: 'New Farm', slug: 'new-farm', description: 'Brisbane\'s brunch capital with a demand-supply gap that is unusual for an inner suburb of this quality. The Brunswick Street and Merthyr Road corridor carries exceptional weekend foot traffic, but only three independent cafés serve it. Riverfront positioning drives aspirational dining. Rents of $4,000–$7,500/month reflect the premium market.', score: bScore('New Farm'), verdict: bVerdict('New Farm'), rentRange: '$4,000–$7,500/mo' },
              { name: 'South Brisbane', slug: 'south-brisbane', description: 'Adjacent to the Gallery of Modern Art, QPAC, and the South Bank cultural precinct, South Brisbane has a tourist-professional mix that generates consistent weekend trade and solid weekday lunchtime demand. The Grey Street and Melbourne Street corridors offer quality positions at $5,500–$10,000/month.', score: bScore('South Brisbane'), verdict: bVerdict('South Brisbane'), rentRange: '$5,500–$10,000/mo' },
            ].map((suburb) => (
              <SuburbCard
                key={suburb.slug}
                name={suburb.name}
