...
import {
  getBrisbaneSuburb,
  getBrisbaneSuburbSlugs,
  getBrisbaneNearbySuburbs,
  type QldSuburb,
} from '@/lib/analyse-data/brisbane'
...
function bestFitLabel(suburb: QldSuburb) {
  const scores = [
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getBrisbaneNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <section style={{ borderBottom: `1px solid ${C.border}`, background: C.white, padding: '56px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 13, color: C.muted, marginBottom: 18 }}>
            ...
            <span>›</span>
            <Link href="/analyse/brisbane" style={{ color: C.brand, textDecoration: 'none' }}>Brisbane</Link>
            <span>›</span>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Brisbane dataset. You can run a full analysis on any address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/brisbane" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>
                Back to Brisbane overview
              </Link>
              <Link href="/onboarding" style={{ background: C.white, color: C.brand, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10, border: `1px solid ${C.border}` }}>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Brisbane suburbs</h2>
...
function SuburbSchema({ suburb }: { suburb: QldSuburb }) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: `${suburb.name} Brisbane Business Location Analysis — ${suburb.verdict} (${suburb.compositeScore}/100)`,
    description: `${suburb.name} scores ${suburb.compositeScore}/100 for Brisbane business suitability with a ${suburb.verdict} verdict for cafés, restaurants, and retail.`,
    author: { '@type': 'Organization', name: 'Locatalyze' },
    publisher: { '@type': 'Organization', name: 'Locatalyze', url: 'https://www.locatalyze.com' },
    dateModified: '2026-04-20',
    about: {
      '@type': 'Place',
      name: `${suburb.name}, Brisbane QLD`,
      address: {
        '@type': 'PostalAddress',
...
export function generateStaticParams() {
  return getBrisbaneSuburbSlugs().map((slug) => ({ suburb: slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { suburb } = await params
  const data = getBrisbaneSuburb(decodeURIComponent(suburb))

  if (!data) {
    return {
      title: 'Brisbane suburb not found — Locatalyze',
      description: 'This Brisbane suburb is not in the current analysis dataset.',
    }
  }

  return {
    title: `${data.name} Brisbane Business Analysis — ${data.verdict} (${data.compositeScore}/100)`,
    ...
    alternates: { canonical: `https://www.locatalyze.com/analyse/brisbane/${data.slug}` },
    openGraph: {
      title: `${data.name} Brisbane Business Analysis`,
      ...
      url: `https://www.locatalyze.com/analyse/brisbane/${data.slug}`,
      type: 'article',
    },
  }
}

export default async function BrisbaneSuburbPage({ params }: Props) {
  const { suburb } = await params
  const data = getBrisbaneSuburb(decodeURIComponent(suburb))
  ...
  const nearby = getBrisbaneNearbySuburbs(data.slug, 3)

  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <SuburbSchema suburb={data} />

      {/* ── Hero — deep teal gradient for Brisbane ── */}
      <section style={{ background: 'linear-gradient(135deg, #0D4D5C 0%, #0E7490 50%, #0891B2 100%)', color: C.white, padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          {/* Breadcrumb */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/brisbane" style={{ color: 'inherit', textDecoration: 'none' }}>Brisbane</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            ...
            <div>
              ...
              <h1 style={{ fontSize: 'clamp(34px, 6vw, 54px)', lineHeight: 1.03, margin: '0 0 14px 0' }}>
                {data.name}
              </h1>
              <p style={{ maxWidth: 560, fontSize: 15, lineHeight: 1.7, color: '#E0F7FF', margin: '0 0 20px 0' }}>
                {data.why[0]}
              </p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                <span style={{
                  ...
                  color: verdict.txt, background: verdict.bg, border: `1px solid ${verdict.bdr}`,
                }}>
                  {data.verdict}
                </span>
            ...
            <div style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.16)', borderRadius: 18, padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 18, marginBottom: 20 }}>
                <div>
                  <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#BAE6FD', margin: '0 0 6px 0' }}>Composite score</p>
                  <div style={{ fontSize: 64, lineHeight: 0.92, fontWeight: 900 }}>{data.compositeScore}</div>
                  <div style={{ fontSize: 13, color: '#BAE6FD', marginTop: 6 }}>out of 100</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#BAE6FD', margin: '0 0 6px 0' }}>Verdict</p>
                  <p style={{ fontSize: 20, fontWeight: 800, lineHeight: 1.2, margin: 0 }}>{data.verdict}</p>
                  <p style={{ fontSize: 13, color: '#BAE6FD', marginTop: 4 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                {[
                  { label: 'Café', value: data.cafe },
                  { label: 'Restaurant', value: data.restaurant },
                  { label: 'Retail', value: data.retail },
                ].map(({ label, value }) => (
                  <div key={label} style={{ textAlign: 'center', padding: '10px 6px', background: 'rgba(255,255,255,0.10)', borderRadius: 10 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#BAE6FD', marginTop: 4 }}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            ...
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 26 }}>
              ...
              <h2 style={{ fontSize: 22, lineHeight: 1.2, color: C.n900, margin: '0 0 6px 0' }}>
                Five-factor model
              </h2>
              <p style={{ fontSize: 13, color: C.muted, lineHeight: 1.6, margin: '0 0 4px 0' }}>
            ...
            <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 26 }}>
              ...
              <h2 style={{ fontSize: 22, lineHeight: 1.2, color: C.n900, margin: '0 0 16px 0' }}>
              ...
              <p style={{ fontSize: 12, color: C.muted, lineHeight: 1.6, margin: '14px 0 0 0' }}>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 26 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: C.brand, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px 0' }}>
              Analyst Notes — {data.name}
            </p>
            <h2 style={{ fontSize: 22, lineHeight: 1.2, color: C.n900, margin: '0 0 18px 0' }}>
              ...
              {data.why.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{
                    ...
                    background: '#E0F7FF', border: '1px solid #BAE6FD',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2,
                  }}>
                    <span style={{ fontSize: 12, fontWeight: 900, color: '#0E7490' }}>{i + 1}</span>
                  </div>
          ...
          {/* CTA */}
          <div style={{ background: 'linear-gradient(135deg, #0D4D5C 0%, #0E7490 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: C.white, margin: '0 0 10px 0' }}>
              Have a specific address in {data.name}?
            </h2>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.85)', margin: '0 0 20px 0', lineHeight: 1.65 }}>
              Run a full competitor map, rent benchmark, and GO/CAUTION/NO verdict for any {data.name} address. Free.
            </p>
            <Link
              href="/onboarding"
              style={{
                display: 'inline-block', padding: '14px 28px',
                backgroundColor: '#BAE6FD', color: '#0D4D5C',
          ...
          {/* Nearby suburbs */}
          <div>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: C.n900, margin: '0 0 16px 0' }}>
              Other Brisbane suburbs to consider
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
              {nearby.map((s) => (
                <SuburbCard
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/brisbane" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>
              ← Back to Brisbane overview
            </Link>
          </div>
        </div>
