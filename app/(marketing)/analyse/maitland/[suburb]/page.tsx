...
import {
  getMaitlandSuburb,
  getMaitlandSuburbSlugs,
  getMaitlandNearbySuburbs,
  type MaitlandSuburb,
} from '@/lib/analyse-data/maitland'
...
function bestFitLabel(suburb: MaitlandSuburb) {
  return [
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getMaitlandNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <section style={{ borderBottom: `1px solid ${C.border}`, background: C.white, padding: '56px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 13, color: C.muted, marginBottom: 18 }}>
            ...
            <span>›</span>
            <Link href="/analyse/maitland" style={{ color: C.brand, textDecoration: 'none' }}>Maitland</Link>
            <span>›</span>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: C.brand, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px 0' }}>Maitland Suburb Intelligence</p>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Maitland dataset. You can run a full analysis on any address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/maitland" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>Back to Maitland overview</Link>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Maitland suburbs</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {alternatives.map((s) => (
              <SuburbCard key={s.slug} citySlug="maitland" name={s.name} slug={s.slug} description={s.why[0]} score={s.compositeScore} verdict={s.verdict === 'RISKY' ? 'NO' : s.verdict} rentRange="" />
            ))}
          </div>
        </div>
      </section>
...
function SuburbSchema({ suburb }: { suburb: MaitlandSuburb }) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: `${suburb.name} Maitland Business Location Analysis — ${suburb.verdict} (${suburb.compositeScore}/100)`,
    description: `${suburb.name} scores ${suburb.compositeScore}/100 for Maitland business suitability with a ${suburb.verdict} verdict for cafes, restaurants, and retail.`,
    author: { '@type': 'Organization', name: 'Locatalyze' },
    publisher: { '@type': 'Organization', name: 'Locatalyze', url: 'https://www.locatalyze.com' },
    dateModified: '2026-04-21',
    about: {
      '@type': 'Place',
      name: `${suburb.name}, Maitland NSW`,
...
export function generateStaticParams() {
  return getMaitlandSuburbSlugs().map((slug) => ({ suburb: slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { suburb } = await params
  const data = getMaitlandSuburb(decodeURIComponent(suburb))
  if (!data) return { title: 'Maitland suburb not found — Locatalyze', description: 'This Maitland suburb is not in the current analysis dataset.' }
  return {
    title: `${data.name} Maitland Business Analysis — ${data.verdict} (${data.compositeScore}/100)`,
    ...
    alternates: { canonical: `https://www.locatalyze.com/analyse/maitland/${data.slug}` },
    openGraph: {
      title: `${data.name} Maitland Business Analysis`,
      description: `${data.name} scores ${data.compositeScore}/100 — cafe ${data.cafe}, restaurant ${data.restaurant}, retail ${data.retail}.`,
      url: `https://www.locatalyze.com/analyse/maitland/${data.slug}`,
      type: 'article',
    },
  }
}

export default async function MaitlandSuburbPage({ params }: Props) {
  const { suburb } = await params
  const data = getMaitlandSuburb(decodeURIComponent(suburb))
  ...
  const nearby = getMaitlandNearbySuburbs(data.slug, 3)
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      ...
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #431407 0%, #9A3412 50%, #C2410C 100%)', color: '#FFFFFF', padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/maitland" style={{ color: 'inherit', textDecoration: 'none' }}>Maitland</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#FFEDD5', margin: '0 0 12px 0' }}>Maitland Suburb Intelligence</p>
      ...
      <section style={{ padding: '32px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
              ...
              {data.why.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ flexShrink: 0, width: 28, height: 28, borderRadius: '50%', background: '#FFEDD5', border: '1px solid #FED7AA', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
                    <span style={{ fontSize: 12, fontWeight: 900, color: '#EA580C' }}>{i + 1}</span>
                  </div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #431407 0%, #9A3412 50%, #C2410C 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
            ...
            <Link href="/onboarding" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: '#FFEDD5', color: '#EA580C', borderRadius: 10, textDecoration: 'none', fontSize: 15, fontWeight: 800 }}>
              Analyse your {data.name} address →
            </Link>
          </div>

          <div>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: C.n900, margin: '0 0 16px 0' }}>Other Maitland suburbs to consider</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
              {nearby.map((s) => (
                <SuburbCard key={s.slug} citySlug="maitland" name={s.name} slug={s.slug} description={s.why[0]} score={s.compositeScore} verdict={s.verdict === 'RISKY' ? 'NO' : s.verdict} rentRange="" />
              ))}
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/maitland" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>← Back to Maitland overview</Link>
          </div>
        </div>
      </section>
    </div>
