...
import {
  getOrangeSuburb,
  getOrangeSuburbSlugs,
  getOrangeNearbySuburbs,
  type OrangeSuburb,
} from '@/lib/analyse-data/orange'
...
function bestFitLabel(suburb: OrangeSuburb) {
  return [
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getOrangeNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <section style={{ borderBottom: `1px solid ${C.border}`, background: C.white, padding: '56px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 13, color: C.muted, marginBottom: 18 }}>
            ...
            <span>›</span>
            <Link href="/analyse/orange" style={{ color: C.brand, textDecoration: 'none' }}>Orange</Link>
            <span>›</span>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: C.brand, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px 0' }}>Orange Suburb Intelligence</p>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Orange dataset. You can run a full analysis on any address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/orange" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>Back to Orange overview</Link>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Orange suburbs</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {alternatives.map((s) => (
              <SuburbCard key={s.slug} citySlug="orange" name={s.name} slug={s.slug} description={s.why[0]} score={s.compositeScore} verdict={s.verdict === 'RISKY' ? 'NO' : s.verdict} rentRange="" />
            ))}
          </div>
        </div>
      </section>
...
function SuburbSchema({ suburb }: { suburb: OrangeSuburb }) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'Article',
    headline: `${suburb.name} Orange Business Location Analysis — ${suburb.verdict} (${suburb.compositeScore}/100)`,
    description: `${suburb.name} scores ${suburb.compositeScore}/100 for Orange business suitability with a ${suburb.verdict} verdict for cafes, restaurants, and retail.`,
    author: { '@type': 'Organization', name: 'Locatalyze' },
    publisher: { '@type': 'Organization', name: 'Locatalyze', url: 'https://www.locatalyze.com' },
    dateModified: '2026-04-21',
    about: {
      '@type': 'Place',
      name: `${suburb.name}, Orange NSW`,
...
export function generateStaticParams() {
  return getOrangeSuburbSlugs().map((slug) => ({ suburb: slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { suburb } = await params
  const data = getOrangeSuburb(decodeURIComponent(suburb))
  if (!data) return { title: 'Orange suburb not found — Locatalyze', description: 'This Orange suburb is not in the current analysis dataset.' }
  return {
    title: `${data.name} Orange Business Analysis — ${data.verdict} (${data.compositeScore}/100)`,
    ...
    alternates: { canonical: `https://www.locatalyze.com/analyse/orange/${data.slug}` },
    openGraph: {
      title: `${data.name} Orange Business Analysis`,
      description: `${data.name} scores ${data.compositeScore}/100 — cafe ${data.cafe}, restaurant ${data.restaurant}, retail ${data.retail}.`,
      url: `https://www.locatalyze.com/analyse/orange/${data.slug}`,
      type: 'article',
    },
  }
}

export default async function OrangeSuburbPage({ params }: Props) {
  const { suburb } = await params
  const data = getOrangeSuburb(decodeURIComponent(suburb))
  ...
  const nearby = getOrangeNearbySuburbs(data.slug, 3)

  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <SuburbSchema suburb={data} />

      <section style={{ background: 'linear-gradient(135deg, #431407 0%, #C2410C 50%, #EA580C 100%)', color: '#FFFFFF', padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/orange" style={{ color: 'inherit', textDecoration: 'none' }}>Orange</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#FFF7ED', margin: '0 0 12px 0' }}>Orange Suburb Intelligence</p>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 14, color: '#FFF7ED' }}>Best fit: {bestFit.label} ({bestFit.value}/100)</span>
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.16)', borderRadius: 18, padding: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 18, marginBottom: 20 }}>
                <div>
                  <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#FFF7ED', margin: '0 0 6px 0' }}>Composite score</p>
                  <div style={{ fontSize: 64, lineHeight: 0.92, fontWeight: 900 }}>{data.compositeScore}</div>
                  <div style={{ fontSize: 13, color: '#FFF7ED', marginTop: 6 }}>out of 100</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#FFF7ED', margin: '0 0 6px 0' }}>Verdict</p>
                  <p style={{ fontSize: 20, fontWeight: 800, lineHeight: 1.2, margin: 0 }}>{data.verdict}</p>
                  <p style={{ fontSize: 13, color: '#FFF7ED', marginTop: 4 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                {[{ label: 'Cafe', value: data.cafe }, { label: 'Restaurant', value: data.restaurant }, { label: 'Retail', value: data.retail }].map(({ label, value }) => (
                  <div key={label} style={{ textAlign: 'center', padding: '10px 6px', background: 'rgba(255,255,255,0.10)', borderRadius: 10 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#FFF7ED', marginTop: 4 }}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section style={{ padding: '32px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              {data.why.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ flexShrink: 0, width: 28, height: 28, borderRadius: '50%', background: '#FFF7ED', border: '1px solid #FED7AA', display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 2 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #431407 0%, #C2410C 50%, #EA580C 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
            ...
            <Link href="/onboarding" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: '#FFF7ED', color: '#EA580C', borderRadius: 10, textDecoration: 'none', fontSize: 15, fontWeight: 800 }}>
              Analyse your {data.name} address →
            </Link>
          </div>

          <div>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: C.n900, margin: '0 0 16px 0' }}>Other Orange suburbs to consider</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
              {nearby.map((s) => (
                <SuburbCard key={s.slug} citySlug="orange" name={s.name} slug={s.slug} description={s.why[0]} score={s.compositeScore} verdict={s.verdict === 'RISKY' ? 'NO' : s.verdict} rentRange="" />
              ))}
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/orange" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>← Back to Orange overview</Link>
          </div>
        </div>
      </section>
    </div>
