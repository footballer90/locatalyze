  ...
  return (
   ...
   <nav style={{ background: S.white, borderBottom: `1px solid ${S.border}`, padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
     ...
     <p style={{ fontSize: 17, color: 'rgba(253,230,138,0.7)', maxWidth: 600, lineHeight: 1.75, marginBottom: 28 }}>Sydney diners spend more per restaurant visit than any other Australian city. The challenge is finding the suburb where that spend is consistent, the rent is defensible, and your concept has room to own a niche.</p>
    ...
    <div style={{ background: '#F8FAFC', border: `1px solid ${S.border}`, borderRadius: 10, padding: '12px 16px', marginBottom: 36, display: 'flex', gap: 10 }}>
     ...
     {TOP_SUBURBS.map((sub, idx) => (
       ...
       <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 20, alignItems: 'start' }}>
        <div>
         ...
         <DataNote text="Income: ABS 2023–24. Rent: CBRE Q4 2025. Profit: Locatalyze model, $250,000 setup, IBISWorld restaurant COGS benchmarks. Daily rev. target = daily revenue required to cover all fixed costs (rent, staff, COGS, overheads) — the point at which the business stops losing money each day."/>
         ...
         <ScoreBar label="Foot traffic" value={sub.footTraffic}/><ScoreBar label="Demographics" value={sub.demographics}/><ScoreBar label="Rent fit" value={sub.rentFit}/><ScoreBar label="Competition" value={sub.competitionScore}/>
        </div>
              </div>
            </div>
          ))}
     ...
     <p style={{ fontSize: 12, color: 'rgba(253,230,138,0.35)', marginTop: 10 }}>No credit card · 1 free preview · ~90 seconds</p>
    </div>
      </div>
    </div>
  )
