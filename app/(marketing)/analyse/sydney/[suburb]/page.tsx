...
import {
  getSydneySuburb,
  getSydneySuburbSlugs,
  getSydneyNearbySuburbs,
  type SuburbModel,
...
function SuburbFallback({ suburbSlug }: { suburbSlug: string }) {
  const alternatives = getSydneyNearbySuburbs('', 3)
  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 18, padding: 32 }}>
            ...
            <p style={{ maxWidth: 680, fontSize: 16, lineHeight: 1.7, color: C.muted, margin: '0 0 22px 0' }}>
              This suburb isn&apos;t in the current Sydney dataset. You can run a full analysis on any address, or explore our scored suburbs below.
            </p>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <Link href="/analyse/sydney" style={{ background: C.brand, color: C.white, textDecoration: 'none', fontWeight: 700, padding: '12px 18px', borderRadius: 10 }}>
      ...
      <section style={{ padding: '40px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, color: C.n900, margin: '0 0 18px 0' }}>Top Sydney suburbs</h2>
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) {
    return {
      title: 'Sydney suburb not found — Locatalyze',
      description: 'This Sydney suburb is not in the current analysis dataset.',
    }
  }
...
export default async function SydneySuburbPage({ params }: Props) {
  ...
  const nearby = getSydneyNearbySuburbs(data.slug, 3)

  return (
    <div style={{ minHeight: '100vh', background: C.n50 }}>
      <SuburbSchema suburb={data} />

      {/* ── Hero — warm amber/gold gradient for Sydney ── */}
      <section style={{ background: 'linear-gradient(135deg, #92400E 0%, #B45309 50%, #D97706 100%)', color: C.white, padding: '56px 24px 52px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          {/* Breadcrumb */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 12, opacity: 0.82, marginBottom: 16 }}>
            ...
            <span>›</span>
            <Link href="/analyse/sydney" style={{ color: 'inherit', textDecoration: 'none' }}>Sydney</Link>
            <span>›</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
            {/* Left */}
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#FDE68A', margin: '0 0 12px 0' }}>
                Sydney Suburb Intelligence
              </p>
              <h1 style={{ fontSize: 'clamp(34px, 6vw, 54px)', lineHeight: 1.03, margin: '0 0 14px 0' }}>
                {data.name}
              </h1>
              <p style={{ maxWidth: 560, fontSize: 15, lineHeight: 1.7, color: '#FEF3C7', margin: '0 0 20px 0' }}>
                {data.why[0]}
              </p>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 14, color: '#FDE68A' }}>
      ...
      <section style={{ padding: '32px 24px 72px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 24 }}>
          ...
          {/* CTA */}
          <div style={{ background: 'linear-gradient(135deg, #92400E 0%, #B45309 100%)', borderRadius: 18, padding: '32px 28px', textAlign: 'center' }}>
          ...
          <div>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: C.n900, margin: '0 0 16px 0' }}>
              Other Sydney suburbs to consider
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
              {nearby.map((s) => (
                <SuburbCard
          ...
          <div style={{ paddingTop: 8 }}>
            <Link href="/analyse/sydney" style={{ fontSize: 14, fontWeight: 700, color: C.brand, textDecoration: 'none' }}>
              ← Back to Sydney overview
            </Link>
          </div>
        </div>
