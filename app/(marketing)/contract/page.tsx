    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: S.n50, fontFamily: S.font }}>
        ...
        <div style={{ background: S.headerBg, padding: '56px 24px 48px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 15, color: '#9CA3AF', maxWidth: 440, margin: '0 auto' }}>We reply to every message within one business day. For urgent issues, email us directly.</p>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '48px 20px 80px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 32, alignItems: 'start' }}>
            ...
            <div style={{ background: S.white, border: `1px solid ${S.n200}`, borderRadius: 18, padding: '32px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
              {sent ? (
                <div style={{ textAlign: 'center', padding: '24px 0' }}>
                  ...
                  <p style={{ fontSize: 14, color: S.n500, marginBottom: 24, lineHeight: 1.7 }}>Thanks {form.name}. We'll get back to you at {form.email} within one business day.</p>
                ...
                <>
                  ...
                  {[
                    ...
                    { label: 'Email address', key: 'email', type: 'email', placeholder: 'jane@yourbusiness.com' },
                  ].map(field => (
                    <div key={field.key} style={{ marginBottom: 16 }}>
                      <label style={{ fontSize: 12, fontWeight: 700, color: S.n700, display: 'block', marginBottom: 6 }}>{field.label}</label>
                  ...
                  <button onClick={handleSubmit} disabled={loading || !form.name || !form.email || !form.message}
                    style={{ width: '100%', padding: '13px', background: (!form.name || !form.email || !form.message) ? S.n200 : S.brand, color: (!form.name || !form.email || !form.message) ? S.n400 : '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: (!form.name || !form.email || !form.message) ? 'not-allowed' : 'pointer', fontFamily: S.font, transition: 'background 0.2s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {[
                { icon: '✉️', title: 'Email us directly', desc: 'hello@locatalyze.com.au', sub: 'We reply within one business day.' },
                { icon: '🕐', title: 'Business hours', desc: 'Monday – Friday, 9am – 5pm AEST', sub: 'We are based in Australia.' },
                { icon: '💬', title: 'Quick questions', desc: 'Check our Help Centre', sub: 'Most common questions are answered there.', link: '/help' },
              ].map(item => (
                <div key={item.title} style={{ background: S.white, border: `1px solid ${S.n200}`, borderRadius: 14, padding: '20px' }}>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <div>
                      ...
                      {item.link
                        ? <Link href={item.link} style={{ fontSize: 14, color: S.brand, fontWeight: 700, textDecoration: 'none' }}>{item.desc}</Link>
                        : <p style={{ fontSize: 14, color: S.brand, fontWeight: 700, marginBottom: 2 }}>{item.desc}</p>}
