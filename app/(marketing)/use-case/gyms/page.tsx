        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '56px 20px 80px' }}>
          ...
          <div style={{ marginBottom: 56 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(290px,1fr))', gap: 16 }}>
              {[
                { icon: '🏘️', color: '#0F766E', bg: '#F0FDFA', title: 'Residential catchment density', body: 'Most gym members come from within 3km. You need 8,000+ households within that radius skewed toward 22–50 year olds with above-average income. High-density apartment precincts are particularly valuable — apartment dwellers are disproportionate gym users.' },
          ...
          <div style={{ background: S.n900, borderRadius: 20, padding: '40px', marginBottom: 48 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12, marginBottom: 28 }}>
              {[['🏘️', 'Residential catchment within 2km and 3km radius'], ['🎯', 'Format gap analysis — what is missing within 2km'], ['💰', 'Membership revenue model at 100/150/200 members'], ['🚗', 'Parking density at peak training hours'], ['✅', 'GO / CAUTION / NO verdict in 30 seconds']].map(([icon, text]) => (
            ...
            <Link href="/auth/signup" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: S.brand, color: '#fff', borderRadius: 10, padding: '13px 26px', fontSize: 14, fontWeight: 800, textDecoration: 'none' }}>Analyse my gym location free →</Link>
          </div>

          <div>
            <p style={{ fontSize: 13, color: S.n500, marginBottom: 14 }}>Browse other business types →</p>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {[['☕', 'Cafes', '/use-case/cafes'], ['🍽️', 'Restaurants', '/use-case/restaurants'], ['👗', 'Retail Stores', '/use-case/retail'], ['🥡', 'Takeaway', '/use-case/takeaway'], ['🏪', 'All Types', '/use-case/all']].map(([icon, label, href]) => (
