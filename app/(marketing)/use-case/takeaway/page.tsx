      ...
      <div style={{ minHeight: '100vh', background: S.n50, fontFamily: S.font }}>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '56px 20px 80px' }}>
          ...
          <div style={{ background: S.n900, borderRadius: 20, padding: '40px', marginBottom: 48 }}>
            ...
            <Link href="/auth/signup" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: S.brand, color: '#fff', borderRadius: 10, padding: '13px 26px', fontSize: 14, fontWeight: 800, textDecoration: 'none' }}>Analyse my takeaway location free →</Link>
          </div>

          <div>
            <p style={{ fontSize: 13, color: S.n500, marginBottom: 14 }}>Browse other business types →</p>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {[['☕', 'Cafes', '/use-case/cafes'], ['🍽️', 'Restaurants', '/use-case/restaurants'], ['👗', 'Retail Stores', '/use-case/retail'], ['💪', 'Gyms', '/use-case/gyms'], ['🏪', 'All Types', '/use-case/all']].map(([icon, label, href]) => (
