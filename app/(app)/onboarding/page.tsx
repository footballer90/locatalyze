    ...
    try {
      const res = await fetch('/api/analyse', {
        ...
        body: JSON.stringify({
          ...
          setupBudget: parseFloat(setup),
          avgTicketSize: parseFloat(ticket),
        ...
        fetch('/api/email', {
          ...
          body: JSON.stringify({
            ...
            report: {
              ...
              breakeven_daily: data.report.breakeven_daily,
              breakeven_months: data.report.breakeven_months,
              result_data: data.report,
            },
            reportId: data.reportId,
          }),
        }).catch(() => {}) // don't block navigation if email fails
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: S.n50, fontFamily: S.font }}>
      ...
      <header style={{ background: S.white, borderBottom: `1px solid ${S.n100}`, padding: '0 24px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <QuotaMeter used={reportsUsed} variant="nav" onUpgrade={() => router.push('/upgrade')} />
      </header>
      ...
      <div style={{ maxWidth: 460, margin: '0 auto', padding: '32px 20px' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <QuotaMeter used={reportsUsed} variant="onboarding" onUpgrade={() => router.push('/upgrade')} />
        </div>
        ...
        {atLimit ? (
          <div style={{ background: S.white, borderRadius: 20, border: `1px solid ${S.n200}`, padding: '32px 28px', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            ...
            <button
              ...
              style={{ background: 'none', border: `1.5px solid ${S.n200}`, borderRadius: 10, padding: '10px 20px', fontSize: 13, fontWeight: 600, color: S.n700 }}
            >
              ← Back to dashboard
            </button>
          </div>
