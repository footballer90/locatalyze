...
// Full-width section wrapper — background bleeds edge to edge
const fullSection = (bg: string, extra = {}) => ({
  width: '100%',
  background: bg,
  ...extra,
})

// Inner content container — max-width centred
const inner = (maxW = 680) => ({
  maxWidth: maxW,
  margin: '0 auto',
  padding: '0 24px',
})
...
export default function LandingPage() {
  ...
  return (
    // ── ROOT: white background, full width, clips horizontal overflow ──
    <div style={{ fontFamily: S.font, background: S.white, color: S.n900, margin: 0, padding: 0, width: '100%', overflowX: 'hidden' }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        html, body { background: #FFFFFF; overflow-x: hidden; width: 100%; }
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 10, background: `linear-gradient(135deg, ${S.brand}, ${S.brandLight})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: S.white, fontWeight: 800, fontSize: 15, boxShadow: '0 2px 8px rgba(15,118,110,0.3)', flexShrink: 0 }}>L</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <Link href="/auth/signup" style={{ fontSize: 13, background: S.brand, color: S.white, borderRadius: 10, padding: '8px 18px', fontWeight: 700, boxShadow: '0 1px 3px rgba(15,118,110,0.3)', whiteSpace: 'nowrap' }}>Get started free</Link>
        </div>
      </nav>

      {/* ── HERO — white background, full width ───────────────────── */}
      <section style={{ ...fullSection(S.white), padding: '72px 0 64px' }}>
        <div style={{ ...inner(680), textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: S.brandFaded, color: S.brand, border: `1px solid ${S.brandBorder}`, borderRadius: 100, padding: '5px 14px', fontSize: 12, fontWeight: 600, marginBottom: 24, letterSpacing: '0.01em' }}>
            <span style={{ width: 6, height: 6, background: S.brandLight, borderRadius: '50%', animation: 'pulse 2s infinite', display: 'inline-block', flexShrink: 0 }} />
      ...
      {/* ── SAMPLE REPORT — white background, full width ──────────── */}
      <section style={{ ...fullSection(S.white), padding: '0 0 80px' }}>
        <div style={{ ...inner(600) }}>
          ...
          <div style={card()}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: S.amberBg, color: S.amber, border: `1.5px solid ${S.amberBorder}`, borderRadius: 100, padding: '5px 14px', fontSize: 12, fontWeight: 700 }}>
                    <span style={{ width: 7, height: 7, borderRadius: '50%', background: S.amber, display: 'inline-block', flexShrink: 0 }} />CAUTION
                  </span>
            ...
            {/* Metrics */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderBottom: `1px solid ${S.n100}` }}>
      ...
      {/* ── SOCIAL PROOF — n50 background, full width ─────────────── */}
      <div style={{ ...fullSection(S.n50), borderTop: `1px solid ${S.n100}`, borderBottom: `1px solid ${S.n100}`, padding: '18px 24px' }}>
      ...
      {/* ── FEATURES — white background, full width ───────────────── */}
      <section style={{ ...fullSection(S.white), padding: '80px 0' }}>
        <div style={{ ...inner(680) }}>
      ...
      {/* ── HOW IT WORKS — n50 background, full width ─────────────── */}
      <section style={{ ...fullSection(S.n50), borderTop: `1px solid ${S.n100}`, borderBottom: `1px solid ${S.n100}`, padding: '80px 0' }}>
        <div style={{ ...inner(600) }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
      ...
      {/* ── TESTIMONIALS — white background, full width ───────────── */}
      <section style={{ ...fullSection(S.white), padding: '80px 0' }}>
        <div style={{ ...inner(680) }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
      ...
      {/* ── DATA SOURCES — n50 background, full width ─────────────── */}
      <section style={{ ...fullSection(S.n50), borderTop: `1px solid ${S.n100}`, borderBottom: `1px solid ${S.n100}`, padding: '60px 0' }}>
      ...
      {/* ── PRICING — white background, full width ────────────────── */}
      <section style={{ ...fullSection(S.white), padding: '80px 0' }}>
        <div style={{ ...inner(680) }}>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14 }}>
            {[
              ...
              <div key={p.name} style={{ ...card({ padding: '24px 20px', border: p.primary ? `2px solid ${S.brand}` : `1px solid ${S.n200}`, position: 'relative' }) }}>
                ...
                <Link href="/auth/signup" style={{ display: 'block', width: '100%', textAlign: 'center', padding: '11px 16px', borderRadius: 10, fontSize: 13, fontWeight: 700, background: p.primary ? S.brand : S.white, color: p.primary ? S.white : S.n700, border: p.primary ? 'none' : `1.5px solid ${S.n200}`, boxSizing: 'border-box' }}>
                  {p.cta}
                </Link>
              </div>
            ))}
      ...
      {/* ── FAQ — n50 background, full width ──────────────────────── */}
      <section style={{ ...fullSection(S.n50), borderTop: `1px solid ${S.n100}`, borderBottom: `1px solid ${S.n100}`, padding: '72px 0' }}>
        <div style={{ ...inner(540) }}>
            ...
            {faqs.map((f, i) => (
              <div key={i} style={{ background: S.white, border: `1px solid ${S.n200}`, borderRadius: 14, overflow: 'hidden' }}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: '100%', textAlign: 'left', padding: '16px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ color: S.n400, fontSize: 20, transform: openFaq === i ? 'rotate(45deg)' : 'none', transition: 'transform 0.2s', lineHeight: 1, flexShrink: 0 }}>+</span>
                </button>
                {openFaq === i && (
                  <div style={{ padding: '12px 20px 16px', fontSize: 13, color: S.n500, lineHeight: 1.7, borderTop: `1px solid ${S.n100}` }}>
                    {f.a}
                  </div>
                )}
              </div>
      ...
      {/* ── FINAL CTA — white background, full width ──────────────── */}
      <section style={{ ...fullSection(S.white), padding: '80px 0' }}>
        <div style={{ ...inner(600) }}>
      ...
      {/* ── FOOTER — white background, full width ─────────────────── */}
      <footer style={{ ...fullSection(S.white), borderTop: `1px solid ${S.n100}`, padding: '28px 24px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 26, height: 26, borderRadius: 8, background: S.brand, display: 'flex', alignItems: 'center', justifyContent: 'center', color: S.white, fontWeight: 700, fontSize: 13, flexShrink: 0 }}>L</div>
