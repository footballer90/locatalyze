...
export default function QuotaMeter({ used, variant, onUpgrade }: QuotaMeterProps) {
  ...
  if (isAtLimit) {
    ...
    <p style={{ fontSize: 12, color: isNearLimit ? S.amber : S.brand, fontWeight: 600 }}>
          {isNearLimit ? 'Last free report' : `Free plan · ${remaining} report${remaining !== 1 ? 's' : ''} remaining`}
    </p>
   ...
   boxShadow: '0 1px 3px rgba(0,0,0,0.04)',
    ...
    <div>
     ...
     <p style={{ fontSize: 14, fontWeight: 800, color: S.n900 }}>
            {isAtLimit ? 'Limit reached' : isNearLimit ? 'Last free report' : `${remaining} report${remaining !== 1 ? 's' : ''} remaining`}
     </p>
        </div>
        <button
          onClick={onUpgrade}
